...
MEMBER_TELEGRAM_MAP = {
    "Bekzod":                  "@Tashkenbayev",
    "Бунёд Умаралиев":         "@bunyod_lv",
    "Vazira":                  "@Vaziira",
    "bohodirusmonov":          "@UsmanovBohodirjon",
    "Humoyun Vaxobov":         "@HumoyunVaxobov",
    "Javoxir Zohidjonov":      "@Zoxidjonov",
    "Alisher Abdusattarov":    "@Alisherabdusattarov",
    "Ravshanbek Vasidjanov":   "@Ravshanbek_Vasidjanov",
}
...
def generate_personal_report(telegram_username):
    ...
    if not target_ids:
        # Debug: Trello dagi haqiqiy ismlarni ko'rsatish
        all_names = list(member_map.values())
        names_str = ", ".join(all_names[:10])
        return (
            f"⚠️ Siz (<b>@{telegram_username}</b>) Trello bilan bog'lanmagan.\n\n"
            f"<b>Trellodagi ismlar:</b> {names_str}\n\n"
            f"<b>Sizning username:</b> @{telegram_username}\n"
            f"<b>Qidirildi:</b> {trello_name or 'topilmadi'}\n\n"
            f"Bot adminiga murojaat qiling."
        )
