...
# ── Xotira fayllari ─────────────────────────────────────
STATE_FILE         = "known_cards.json"
WARNED_5DAYS_FILE  = "warned_5days.json"
WARNED_OVERDUE_FILE= "warned_overdue.json"
WARNED_STUCK_FILE  = "warned_stuck.json"
WARNED_NODUE_FILE  = "warned_nodue.json"
WARNED_NOUSER_FILE = "warned_nouser.json"
CARD_SEEN_DATE_FILE= "card_seen_date.json"
...
def check_trello_changes():
    ...
    for card in cards:
        ...
        if within and warn_key_5 not in warned_5days:
            d = due_date(card)
            send_message(
        ...
        if is_overdue(card, list_name) and warn_key_od not in warned_overdue:
            d = due_date(card)
