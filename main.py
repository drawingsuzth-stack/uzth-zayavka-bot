...
def get_card_label(card):
    """Kartaning zayavka turini custom fields orqali qaytaradi."""
    field_id = get_zayavka_turi_field_id()
    if not field_id:
        return None
    
    options = _custom_field_cache.get("zayavka_turi_options", {})
    ...
    for item in custom_field_items:
        if item.get("idCustomField") == field_id:
            ...
            if option_id and option_id in options:
                val = options[option_id]
                # Mahalliy, Import, Aralash ni tekshirish
                for label in [LABEL_MAHALLIY, LABEL_IMPORT, LABEL_ARALASH]:
